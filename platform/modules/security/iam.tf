...
resource "aws_iam_user" "maintainer-1" {
  name = "${var.app}-${var.environment}-${var.maintainer-1}"
  tags = {
    app = "${var.app}"
  }
}
resource "aws_iam_user" "maintainer-2" {
  name = "${var.app}-${var.environment}-${var.maintainer-2}"
  tags = {
    app = "${var.app}"
  }
}
...
resource "aws_iam_group" "iam-group-maintainers" {
  name = "${var.app}-${var.environment}-group-maintainers"
}
...
resource "aws_iam_role" "iam-role-maintainer" {
  name = "${var.app}-${var.environment}-role-maintainer"
...
resource "aws_iam_policy" "iam-policy-users-all" {
  name        = "${var.app}-${var.environment}-policy-users-all"
...
resource "aws_iam_policy" "iam-policy-logging" {
  name        = "${var.app}-${var.environment}-policy-logging"
...
resource "aws_iam_policy" "iam-policy-buckets-event-function" {
  name        = "${var.app}-${var.environment}-policy-buckets-event-function"
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Resource": ["arn:aws:s3:::${var.s3-bucket-entities_id}","arn:aws:s3:::${var.s3-bucket-entities_id}/*"],
      "Action": "s3:*"
    }
    , {
      "Effect": "Allow",
...
resource "aws_iam_policy" "iam-policy-maintainers" {
  name        = "${var.app}-${var.environment}-policy-maintainers"
